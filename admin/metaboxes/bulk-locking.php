<?php
	 ...
	 * @return string
	 */
	function onp_bzda_adn_bulk_locking_way_state_class($wayStateClass, $options)
	{
	...
	add_filter('bizpanda_bulk_locking_way_state_class', 'onp_bzda_adn_bulk_locking_way_state_class', 10, 2);
	 ...
	 * @return array
	 */
	function onp_bzda_adn_bulk_locking_allow_save_ways($ways)
	{
	...
	add_filter('bizpanda_bulk_locking_allow_save_ways', 'onp_bzda_adn_bulk_locking_allow_save_ways');
	add_filter('bizpanda_bulk_locking_allow_clear_ways', 'onp_bzda_adn_bulk_locking_allow_save_ways');

	/**
	 * Добавляет новую закладку в массов блокировщике
	 * @param $defaultWay
	 */
	function onp_bzda_adn_bulk_locking_add_tab_before($defaultWay)
	{
		?>
		<button data-target="#onp-sl-fullscreen-lock-options" type="button" class="btn btn-default value fullscreen-lock <?php if( $defaultWay == 'fullscreen-lock' ) {
			...
			<i class="fa fa-arrows-alt" aria-hidden="true"></i><?php _e('Fullscreen Lock', 'plugin-addon-popup-locker'); ?>
		</button>
	<?php
	}

	add_action('bizpanda_bulk_locking_add_tab_before', 'onp_bzda_adn_bulk_locking_add_tab_before');
	 ...
	 * @param $types
	 */
	function onp_bzda_adn_bulk_locking_add_tab_content($options, $defaultWay, $types)
	{
		...
		<div id="onp-sl-fullscreen-lock-options" class="onp-sl-bulk-locking-options <?php if( $defaultWay !== 'fullscreen-lock' ) {
			...
			<div class="onp-sl-description">
				<?php _e('Every page will be full hidden under locker, with the background.', 'plugin-addon-popup-locker') ?>
			</div>
			<div class="onp-sl-content">
				<div class="onp-sl-form">
					<div class='onp-sl-row' style="margin-bottom: 30px;">
						<label><?php _e('Only selected Posts', 'plugin-addon-popup-locker') ?></label>
						...
						<div class="help-block">
							<?php _e('(Optional) Enter posts IDs comma separated, for example, "19,25,33".', 'plugin-addon-popup-locker') ?>
						</div>
					</div>
					<div class="onp-sl-limits">
						<div class='onp-sl-exclude'>
							<div class='onp-sl-row'>
								<label><?php _e('Exclude Posts', 'plugin-addon-popup-locker') ?></label>
							...
							<div class='onp-sl-row'>
								<label><?php _e('Exclude Categories', 'plugin-addon-popup-locker') ?></label>
								...
								<div class="help-block">
									<?php _e('(Optional) Enter categories IDs comma separated, for example, "4,7".', 'plugin-addon-popup-locker') ?>
								</div>
							</div>
						</div>
						<div class='onp-sl-post-types'>
							<strong><?php _e('Posts types to lock', 'plugin-addon-popup-locker') ?></strong>

							<div class="help-block">
								<?php _e('Choose post types for batch locking.', 'plugin-addon-popup-locker') ?>
							</div>
							<ul>
							...
							<div class="help-block help-block-error">
								<?php _e('Please choose at least one post type to lock. Otherwise, nothing to lock.', 'plugin-addon-popup-locker') ?>
							</div>
						</div>
					</div>
				</div>
	...
	add_action('bizpanda_bulk_locking_add_tab_content', 'onp_bzda_adn_bulk_locking_add_tab_content', 10, 3);

	function onp_bzda_adn_bulk_locking_add_way_description()
	{
		?>
	...
	add_action('bizpanda_bulk_locking_add_way_description', 'onp_bzda_adn_bulk_locking_add_way_description');
